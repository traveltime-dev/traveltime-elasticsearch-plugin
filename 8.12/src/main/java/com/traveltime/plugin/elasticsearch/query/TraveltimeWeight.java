...
public class TraveltimeWeight extends Weight {
  @Getter private final TraveltimeSearchQuery ttQuery;
  ...
  @EqualsAndHashCode.Exclude private final ProtoFetcher protoFetcher;

  public TraveltimeWeight(
      TraveltimeSearchQuery q, Weight prefilter, boolean hasOutput, float boost) {
    super(q);
    ttQuery = q;
    ...
    protoFetcher =
        FetcherSingleton.INSTANCE.getFetcher(
            q.getAppUri(), q.getAppId(), q.getApiKey(), SpecialPermission::new);
  }
  ...
  private FilteredIterator filteredValues(LeafReaderContext context) throws IOException {
    ...
    if (prefilter != null) {
      ...
      finalIterator = ConjunctionUtils.intersectIterators(List.of(prefilterIterator, backing));
    } else {
